...
function loadOrderSummary() {
  ...
  if (cart.length === 0) {
    itemsList.innerHTML = '<p style="color: var(--text-gray); padding: 20px;">Giỏ hàng trống</p>';
    return;
  }
  ...
  cart.forEach(item => {
    ...
    const customization = [optionsText, toppingsText].filter(Boolean).join(' | ');
  ...
  document.getElementById('taxAmount').textContent = Math.round(tax).toLocaleString('vi-VN') + 'đ';
  document.getElementById('totalAmount').textContent = Math.round(total).toLocaleString('vi-VN') + 'đ';
}
